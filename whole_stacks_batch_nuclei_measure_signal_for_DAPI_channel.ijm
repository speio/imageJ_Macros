...
function area_measure() {	
	...
	selectWindow("C3-"+imageTitle);
	run("Set Measurements...", "area mean standard modal min integrated median kurtosis stack display add decimal=3 redirect=C3-"+imageTitle);
	n = nSlices;
	print(n+"slices");
	for (i=1; i <=n; i++) {
		selectWindow("C3-"+imageTitle);
		...
		run("Find Maxima...", "prominence=180 strict light output=Count"); // Count cells
		selectWindow(maskImageTitle); 
		run("Analyze Particles...", "size=10-Infinity show=Outlines summarize display add");
		selectWindow("C3-"+imageTitle);
		run("Find Maxima...", "prominence=180 strict output=Count"); // Count Maxima
	}
